...
resource "null_resource" "update_permissions" {
  provisioner "local-exec" {
    command = <<EOT
      echo '${tls_private_key.mykey.private_key_openssh}' > ./web-key-prod-ec2.pem
      chmod 400 ./web-key-prod-ec2.pem
    EOT
  }
  triggers = {
...
resource "aws_ssm_parameter" "ec2_private_key" {
  name        = "/capstone/project1/prod_ec2_private_key"
...
resource "aws_ssm_parameter" "ec2_public_key" {
  name        = "/capstone/project1/prod_ec2_public_key"
